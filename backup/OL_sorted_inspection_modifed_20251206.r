...
path_GSE160512 <- file.path("/mnt","workingA","BMP4_OL_sorted","reference","GSE160512","raw")
...
GSE160512.meta <- read.table("/mnt/workingA/BMP4_OL_sorted/reference/GSE160512/GSE160512_PS2APPTsneCoordinatesAndClusterAssignments.txt.gz", header =T, sep="\t")
...
path_GSE153895 <- file.path("/mnt","workingA","BMP4_OL_sorted","reference","GSE153895","raw")
...
GSE153895.meta <- read.table("/mnt/workingA/BMP4_OL_sorted/reference/GSE153895/GSE153895_TsneCoordinatesAndClusterAssignments.txt.gz", header =T, sep="\t")
...
path_GSE148676 <- file.path("/mnt","workingA","BMP4_OL_sorted","reference","GSE148676")
...
GSE148676.merged <- GSE148676.merged %>%
                          FindClusters(resolution = seq(0.3,1,0.1), algorithm = 2)
...
GSE148676.oligo <- subset(GSE148676.merged, idents = c(1, 3, 7, 8, 25))
...
data <- lapply(str_sub(list.files("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/", pattern = "_matrix.mtx.gz"), 1, str_locate(list.files("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/", pattern = "_matrix.mtx.gz"),"_matrix.mtx.gz")[,1]), function(x) {
    Read10X_GEO("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/",x)
})
...
for(i in seq_len(length(GSE266687.obj))){
    GSE266687.obj[[i]]@meta.data$id <- str_sub(list.files("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/", pattern = "_matrix.mtx.gz"), 1, 10)[i]
    GSE266687.obj[[i]]@meta.data$basic <- "GSE266687"
    GSE266687.obj[[i]]@meta.data$type <- str_sub(list.files("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/", pattern = "_matrix.mtx.gz"), 12, str_locate(list.files("/mnt/workingA/BMP4_OL_sorted/reference/GSE266687/", pattern = "_matrix.mtx.gz"),"_matrix.mtx.gz")[,1]-1)[i]
...
GSE266687.oligo <- subset(GSE266687.merged, idents = c(3, 12, 14, 19))
...
GSE278199 <- readRDS("/mnt/workingA/BMP4_OL_sorted/reference/GSE278199/GSE278199_OPC_cells.rds")
...
# save.image("/mnt/workingA/BMP4_OL_sorted/final_Rdata/OL_sorted_2025-12-06 10-04-14.Rdata")
load("/mnt/workingA/BMP4_OL_sorted/final_Rdata/OL_sorted_2025-12-06 10-04-14.Rdata")

#GSE266687.oligo, 
OL.merge <- merge(GSE278199.obj, y= c(GSE153895.oligo, GSE160512.oligo), project = 'merged',merge.data=FALSE) #GSE148676.oligo.subset, 
...
#GSE266687.oligo
OL.features <- SelectIntegrationFeatures(object.list = c(GSE278199.obj, GSE153895.oligo, GSE160512.oligo), nfeatures = 3000) #GSE148676.oligo.subset, 
...
DimPlot(OL.merged, #subset(OL.merged, ident = setdiff(levels(Idents(OL.merged) ), c(22:25))), 
        # cols = pal_npg("nrc")(10),
        # group.by = "celltype", 
        reduction = "umap",
        label = T, repel = T, 
...
OL.mergedd <- subset(OL.merged, ident = setdiff(levels(Idents(OL.merged) ), c(22, 24:25)))
OL.mergedd <- PrepSCTFindMarkers(OL.mergedd)

require(ggsci)

p0s<-DimPlot(OL.mergedd, 
        cols = pal_npg("nrc")(10),
...
p1s<-DimPlot(subset(OL.mergedd, subset = id == names(table(OL.mergedd$id))[7] | 
                           id == names(table(OL.mergedd$id))[8] | 
                           id == names(table(OL.mergedd$id))[9] | 
                           id == names(table(OL.mergedd$id))[13] | 
                           id == names(table(OL.mergedd$id))[14] | 
                           id == names(table(OL.mergedd$id))[15] | 
                           id == names(table(OL.mergedd$id))[16] | 
                           id == names(table(OL.mergedd$id))[17] | 
                           id == names(table(OL.mergedd$id))[18] | 
                           id == names(table(OL.mergedd$id))[19] | 
                           id == names(table(OL.mergedd$id))[20] | 
                           id == names(table(OL.mergedd$id))[21] | 
                           id == names(table(OL.mergedd$id))[22] | 
                           id == names(table(OL.mergedd$id))[23] | 
                           id == names(table(OL.mergedd$id))[24] ), 
        cols = pal_npg("nrc")(10),
        group.by = "new_cell_type", 
        # group.by = "basic", 
        reduction = "umap", 
...
p1s_1<-DimPlot(subset(OL.mergedd, subset = id == names(table(OL.mergedd$id))[7] | 
                           id == names(table(OL.mergedd$id))[8] | 
                           id == names(table(OL.mergedd$id))[9] | 
                           id == names(table(OL.mergedd$id))[13] | 
                           id == names(table(OL.mergedd$id))[14] | 
...
theme(legend.position = "bottom",  
        ...
        plot.title = element_text(size = 14, face = "bold",hjust = 0), axis.line = element_blank(), 
        axis.text = element_blank(),  axis.ticks = element_blank()) + 
  ggtitle(paste0("OL - cells : ",comma(length(colnames(OL.mergedd)),format="d")," cells")) 

PS2APP <- subset(OL.mergedd, subset = id == names(table(OL.mergedd$id))[7] | 
                           id == names(table(OL.mergedd$id))[8] | 
                           id == names(table(OL.mergedd$id))[9] | 
                           id == names(table(OL.mergedd$id))[13] | 
                           id == names(table(OL.mergedd$id))[14] | 
...
write.xlsx(COP.markers, "/mnt/workingA/BMP4_OL_sorted/results/fig1S_b_new_20251208.xlsx", rowNames = TRUE)
...
ggsave("/mnt/workingA/BMP4_OL_sorted/results/fig1S_c_new_20251208.pdf",p1s, width = 7.5, height = 7.5, units= "in" )
ggsave("/mnt/workingA/BMP4_OL_sorted/results/fig1S_d_new_20251208.pdf",p1s_1, width = 7.5, height = 7.5, units= "in" )
ggsave("/mnt/workingA/BMP4_OL_sorted/results/fig1S_a_new_20251208.pdf",p0s, width = 7.5, height = 7.5, units= "in" )
ggsave("/mnt/workingA/BMP4_OL_sorted/results/fig1S_e_new_20251208.pdf",p0s_1, width = 7.5, height = 7.5, units= "in" )

require(EnhancedVolcano)
pdf("/mnt/workingA/BMP4_OL_sorted/results/fig1S_b_new_20251208.pdf", width = 5, height = 7.5, family = "ArialMT")
    EnhancedVolcano(COP.markers,
                        ...
                        selectLab = c("Bcas1","Gpr17","Enpp6","Bmp4"), 
...
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "4"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "6"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "12"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "14"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "18"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "19"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "23"))] <- "OPC"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "11"))] <- "COP"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "21"))] <- "COP"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "0"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "1"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "2"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "3"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "5"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "7"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "8"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "9"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "10"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "13"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "15"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "16"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "17"))] <- "Oligodendrocytes"
OL.mergedd@meta.data$new_cell_type[colnames(OL.mergedd) %in% colnames(subset(OL.mergedd, idents = "20"))] <- "Oligodendrocytes"
...
# load("/mnt/workingA/BMP4_OL_sorted/final_Rdata/OL_sorted_new_20240103.RData")
# names(table(OL.COP@meta.data$genetype))

table(OL.COP@meta.data$basic)

table(OL.COP@meta.data$genetype)

OL.COP@meta.data$genetype[OL.COP@meta.data$basic == "GSE278199" & is.na(OL.COP@meta.data$genetype)] <- "WT/CPZ"
...
OL.COP.DEP.DM_with_CPZ3 <- FindMarkers(OL.COP, 
                                        ident.1 = names(table(OL.COP@meta.data$genetype))[1],
...
OL.COP.DEP.PS2_APP <- FindMarkers(OL.COP, 
                                        ...
                                        ident.2 = names(table(OL.COP@meta.data$genetype))[8],
                                        group.by = "genetype", 
                                        test.use = "MAST",
                                        recorrect_umi = FALSE)

OL.COP.DEP.tau <- FindMarkers(OL.COP, 
                                        ident.1 = names(table(OL.COP@meta.data$genetype))[6],
                                        ident.2 = names(table(OL.COP@meta.data$genetype))[8],
                                        group.by = "genetype", 
                                        test.use = "MAST",
                                        recorrect_umi = FALSE)
...
write.xlsx(OL.COP.DEP.DM_with_CPZ3, "/mnt/workingA/BMP4_OL_sorted/results/fig1S_CPZ3_20251209.xlsx", rowNames = TRUE)
write.xlsx(OL.COP.DEP.PS2_APP, "/mnt/workingA/BMP4_OL_sorted/results/fig1S_PS2APP_20251209.xlsx", rowNames = TRUE)
write.xlsx(OL.COP.DEP.tau, "/mnt/workingA/BMP4_OL_sorted/results/fig1S_P301L_20251209.xlsx", rowNames = TRUE)

library(dplyr)
library(tibble)
library(pheatmap)

# 1. 보고 싶은 유전자 리스트 정의 (순서 고정)
target_genes <- c("Bmp4", "Enpp6", "Bcas1", "Gpr17", "Cnp", "Cyfip2", 
                  "Lims2", "Wasf1", "Phyhipl", "Nfasc", "Frmd4a", "Olig2", "Pdgfa")

# 2. 데이터 추출 및 병합 함수 생성
# 각 DE 결과에서 avg_log2FC만 뽑아오는 함수입니다.
extract_logFC <- function(de_results, col_name) {
  de_results %>%
    rownames_to_column("Gene") %>%
    dplyr::select(Gene, avg_log2FC) %>%
    dplyr::rename(!!col_name := avg_log2FC) # 동적으로 컬럼 이름 변경
}

# 3. 각 비교군 데이터 준비
# (이미 위에서 FindMarkers로 생성된 객체들을 사용합니다)
...
# 4. 데이터 병합 (Left Join을 사용하여 유전자 순서 유지)
# 기준이 되는 target_genes 데이터프레임을 먼저 만듭니다.
...
# 5. 데이터 정리
# Gene 컬럼을 행 이름으로 설정하고 제거
rownames(heatmap_data) <- heatmap_data$Gene
heatmap_data <- heatmap_data[, -1]

# NA 처리: DE 분석에서 유의하지 않거나 발현이 없어 빠진 유전자는 log2FC를 0으로 처리
heatmap_data[is.na(heatmap_data)] <- 0

# 6. Heatmap 그리기
# cluster_rows/cols = FALSE로 설정하여 지정한 순서 그대로 출력합니다.
# pheatmap(heatmap_data,
#          cluster_rows = FALSE,    # 유전자 순서 고정 (입력한 순서대로)
#          cluster_cols = FALSE,    # 비교군 순서 고정 (CPZ, P301L, PS2APP)
#          display_numbers = FalAW,  # 셀 안에 수치 표시 (선택 사항)
#          color = colorRampPalette(c("blue", "white", "red"))(100), # 색상 테마
#          breaks = seq(-2, 2, length.out = 101), # 색상 범위를 -2 ~ 2로 고정 (필요시 조절)
#          main = "Log2FC Heatmap: OL.COP Markers")

pdf("/mnt/workingA/BMP4_OL_sorted/results/fig1S_c_20251209.pdf", width = 5, height = 15, family = "ArialMT")
...
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2b_Dim_20230720.jpeg"), p1a, width = 7.5, height = 7.5, units = "in", device = "jpeg", dpi = 300)
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2c_Dim_20230720.jpeg"), p1b, width = 7.5, height = 7.5, units = "in", device = "jpeg", dpi = 300)
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2d_Den_20230720.jpeg"), p1c, width = 16, height = 12, units = "in", device = "jpeg", dpi = 300)
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/SuppFig_1a_Dim_20230720.jpeg"), pS1a, width = 7.5, height = 7.5, units = "in", device = "jpeg", dpi = 300)
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/SuppFig_1b_Dot_20230720.jpeg"), pS1b, width = 15, height = 10, units = "in", device = "jpeg", dpi = 300)
...
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2d_score_20230720.jpeg"), 
        pbox_MYL[[6]] + pbox_MYL[[3]] + ylab("") + pbox_MYL[[5]]+ ylab(""), width = 20, height = 7.5, units = "in", device = "jpeg", dpi = 300)

ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2e_VC_20230720.jpeg"), 
...
ggsave(paste0(file.path("/home","choelab","working","BMP4_OL_sorted","results"),"/Figure2f_GO-BP_20230720.jpeg"), 
